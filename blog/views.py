...
def myservice(request):
	return render(request, 'blog/myservice.html')
...
def post_detail_ad(request, pk):
    a = get_object_or_404(Post_Admin, pk=pk)
    return render(request, 'blog/post_detail_ad.html', {'post_ad': a})

def post_detail_pg(request, pk):
    a = get_object_or_404(Post_Program, pk=pk)
...
def leave_comment_ad(request, pk):
	...
	return HttpResponseRedirect( reverse('blog:post_detail_ad', args = (pk,)) )
